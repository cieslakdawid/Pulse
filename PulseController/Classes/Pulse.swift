...
public class Pulse: NSObject {
    ...
    required public init(configuration: Configuration, measureClosure: @escaping (() -> CGFloat), outputClosure: @escaping ((_ output: CGFloat) -> Void)) {
        ...
        self.displayLink = CADisplayLink(target: displayLinkProxy, selector: #selector(displayLinkDidFire))
        super.init()
    ...
    @objc func displayLinkDidFire() {
        ...
        let dt: TimeInterval = currentTime - previousTickTime
        tick(dt: dt)
    }
    ...
    private func calculateOutput(_ dt: TimeInterval) {
